...
int main(int argc, char ** argv) {
    ...
    std::vector<llama_token> embd_inp; // OXW20260416 input embeddings ??

    LOG("  chat_add_and_format CAAF 1\n"); // outside while loop line 547
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        if (!embd.empty()) {
            ...
            for (int i = 0; i < (int) embd.size(); i += params.n_batch) {
                ...
                // OXW20260416 one token per line
                // LOG_DBG("n_past = %d\n", n_past);
                LOG_DBG("n_past = %d  ", n_past);
        ...
        if ((int) embd_inp.size() <= n_consumed) {
            ...
            if (llama_token_is_eog(model, common_sampler_last(smpl))) {
                ...
                LOG("  chat_add_and_format CAAF 2\n"); // inside while loop 
            ...
            if (n_past > 0 && is_interacting) {
                ...
                if (buffer.length() > 1) {
                    ...
                    LOG("  chat_add_and_format CAAF 3\n"); // inside while loop 
